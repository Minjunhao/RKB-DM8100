...
MAC_INITIAL_TYPE PHY_initial_status=PHY_RESET;
PHY_ADDR_RWTYPE phy_addr_RW=PHY_ADDR_WRITE;
...
void ETH_StructInit(ETH_InitTypeDef* ETH_InitStruct)
{
  ...
  ETH_InitStruct->ETH_PassControlFrames = ETH_PassControlFrames_ForwardPassedAddrFilter;   
...
/**
  ...
  *         ETH_SUCCESS: Ethernet successfully initialized                 
  */
uint32_t ETH_Init_no_loop(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  uint32_t RegValue = 0, tmpreg = 0;
  __IO uint32_t i = 0;
  RCC_ClocksTypeDef  rcc_clocks;
  ...
  //_eth_delay_(PHY_RESET_DELAY);
  PHY_initial_status=PHY_RESET_DELAY_CHECK;
  timeout=0;             
  break;  
  case PHY_RESET_DELAY_CHECK:
  	if(++timeout==1000)
  		{
          PHY_initial_status=PHY_LINK_STATUS_CHECKING;
          timeout=0;   
	    }
  break;
  case PHY_LINK_STATUS_CHECKING:
    timeout++;
  if(ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
  {  
    /* We wait for linked status... */
	if((ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_Linked_Status) && (timeout < PHY_READ_TO))
		{
		    if(!(ETH_WritePHYRegister(PHYAddress, PHY_BCR, PHY_AutoNegotiation)))
               {
                   /* Return ERROR in case of write timeout */
             ...
             PHY_initial_status=PHY_ATUO_NEG_CHECKING;
			 timeout = 0; 
	    }
	else
		{
   ...
   break;
   case PHY_ATUO_NEG_CHECKING:
	timeout++;
	if((ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_AutoNego_Complete) && (timeout < (uint32_t)PHY_READ_TO))
		{
            PHY_initial_status=PHY_MAC_SETTING;
			timeout = 0;
	    }
	else
		{
...
uint32_t ETH_WritePHYRegister(uint16_t PHYAddress, uint16_t PHYReg, uint16_t PHYValue)
{
  ...
  /* Check for the Busy flag */
  break;
  case PHY_ADDR_WAIT_RESPONSE:
  	timeout++;
	tmpreg = ETH->MACMIIAR;
	if(!(tmpreg & ETH_MACMIIAR_MB) && (timeout < (uint32_t)PHY_WRITE_TO))
		{
		   phy_addr_RW=PHY_ADDR_WRITE;
           ret=ETH_SUCCESS; 
	    }
    if(timeout == PHY_WRITE_TO)
     {
